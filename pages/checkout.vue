...
const { cartGrandTotal, cartItems } = storeToRefs(useCartStore());
		...
		<div class="container flex flex-column items-start">
			...
			<div class="checkout-page__content grid items-start">
				...
				<div class="checkout-page__summary bg-white br-8 flex flex-column">
					...
					<BaseButton v-if="cartItems.length > 0" size="full" :disabled="v$.$invalid" @click="setCurrentModal('order')">continue & pay</BaseButton>
				</div>
			</div>
		</div>
	</div>
